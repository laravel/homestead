...
class Homestead
  def self.configure(config, settings)
    ...
    config.vm.provision 'shell' do |s|
      ...
      s.inline = 'sudo chown -R vagrant:vagrant /usr/local/bin && sudo -u vagrant /usr/local/bin/composer self-update --2 --no-progress && sudo chown -R vagrant:vagrant /home/vagrant/.composer/'
      s.privileged = false
    end
